...
const Navbar = () => {
  ...
  return (
    <div className="fixed top-0 right-0 left-0 z-50 mx-4 sm:mx-[10%] flex justify-between items-center text-sm py-4 mb-5 border-b border-gray-400 bg-white">
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed top-0 right-0 h-full w-3/4 bg-white text-gray-800 shadow-lg z-40 transform transition-transform duration-300 ease-in-out md:hidden ${showMenu ? 'translate-x-0' : 'translate-x-full'}`}
      >
      ...
      {/* Overlay */}
      <div
        className={`fixed inset-0 bg-black transition-opacity duration-300 ${showMenu ? 'opacity-50' : 'opacity-0 pointer-events-none'}`}
        onClick={() => setShowMenu(false)}
      ></div>
    </div>
  );
